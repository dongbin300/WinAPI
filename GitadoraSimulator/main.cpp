...
void Display(HWND hWnd)
{
	...
	{
		char Tpt[20], Tbpm[20], Tfps[20];
		sprintf(Tpt, "TIME: %d", game.play.time);
		sprintf(Tbpm, "BPM: %d", game.music[game.choice.ptr].bpm);
		sprintf(Tfps, "FPS: %.2f", fps);
		__text(Tpt, 100, 200);
		__text(Tbpm, 100, 220);
		__text(Tfps, 100, 240);
...
void GameRunMain(HWND hWnd)
{
	// 마지막 프레임으로부터 경과된 시간을 계산해 frameTime에 저장한다
	QueryPerformanceFrequency(&timerFreq);
	QueryPerformanceCounter(&timeEnd);
	frameTime = (float)(timeEnd.QuadPart - timeStart.QuadPart) / (float)timerFreq.QuadPart;

	// 절전을 위한 코드, winmm.lib 가 필요하다
	// 경과된 시간을 원하는 프레임 속도에 충족되지 않는다면
	if (frameTime < MIN_FRAME_TIME)
	{
		sleepTime = (double)((MIN_FRAME_TIME - frameTime) * 1000000);
		timeBeginPeriod(1);        // 윈도우 타이머를 위한 1ms 해상도 요청
		USleep(sleepTime);		 // CPU를 sleepTime만큼 쉬게 한다
	...
	if (frameTime > MAX_FRAME_TIME)		// 프레임 속도가 매우 느리다면
		frameTime = MAX_FRAME_TIME;		// 최대 frameTime으로 제한한다
	timeStart = timeEnd;
